...
def train_and_score_fold(
    ...
    if len(pred_ds_aligned) != len(valid_ds) or not np.array_equal(pred_ds_aligned, valid_ds):
        raise ValueError("PatchTST prediction dates do not align with validation dates. Check embargo/frequency.")
...
def tune_patchtst(
    ...
    patch_cfg = cfg.get("patchtst", {})
    train_cfg = patch_cfg.get("training", {})
    tuning_cfg = patch_cfg.get("tuning", {})
    search_cfg = patch_cfg.get("search_space", {})
    ...
    use_exog = bool(patch_cfg.get("use_exog", False))
    if not use_exog:
        exog_cols = []
    ...
    study.optimize(objective, n_trials=tuning_cfg.get("n_trials", 20), timeout=tuning_cfg.get("timeout_seconds", 0) or None)

    best_params = study.best_params
    (artifacts["reports"] / "patchtst_best_params.json").write_text(
        json.dumps(best_params, indent=2), encoding="utf-8"
    )
    (artifacts["reports"] / "patchtst_best_value.json").write_text(
...
def predict_patchtst(
    ...
    params_path = artifacts["reports"] / "patchtst_best_params.json"
    if not params_path.exists():
        raise FileNotFoundError("patchtst_best_params.json not found. Run PatchTST tuning first.")
    ...
    out = pd.DataFrame(
        {
            date_col: date_vals,
            "patchtst_pred": y_pred,
        }
    )

    out_path = artifacts["preds"] / "patchtst_test_predictions.csv"
