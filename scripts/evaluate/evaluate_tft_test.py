...
def main() -> None:
    ...
    pred_path = artifacts["preds"] / "tft_test_predictions.csv"

    if not pred_path.exists():
        raise FileNotFoundError("tft_test_predictions.csv not found. Run scripts/predict/predict_tft.py first.")
    ...
    merged = pd.merge(test_df[[date_col, target_col]], pred_df[[date_col, "tft_pred"]], on=date_col, how="inner")
    merged = merged.sort_values(date_col).reset_index(drop=True)

    metrics = compute_fold_metrics(
        y_true=merged[target_col].values,
        pred=merged["tft_pred"].values,
        date_id=merged[date_col].values,
        sf=cfg["evaluation"]["sharpe"]["annualization"],
        nw_lag=cfg["evaluation"]["sharpe"]["nw_lag"],
    )

    daily = merged.groupby(date_col, sort=True)[[target_col, "tft_pred"]].mean()
    strat = np.sign(daily["tft_pred"]) * daily[target_col]
    buy_hold = daily[target_col]

    curve_df = pd.DataFrame(
        {
            date_col: daily.index.values,
            "cum_tft": ((1 + strat).cumprod() - 1).values,
            "cum_buy_hold": ((1 + buy_hold).cumprod() - 1).values,
        }
    )

    curve_csv = artifacts["reports"] / "tft_test_curve.csv"
    curve_png = artifacts["reports"] / "tft_vs_buyhold_test_curve.png"
    metrics_json = artifacts["reports"] / "tft_test_metrics.json"

    curve_df.to_csv(curve_csv, index=False)
    metrics_json.write_text(json.dumps(metrics, indent=2), encoding="utf-8")

    plt.figure(figsize=(10, 4))
    plt.plot(curve_df[date_col], curve_df["cum_tft"], label="TFT (sign pred)")
    plt.plot(curve_df[date_col], curve_df["cum_buy_hold"], label="Buy & Hold")
    plt.xlabel(date_col)
    plt.ylabel("Cumulative return")
    plt.title("Test Set: TFT vs Buy & Hold")
    plt.legend()
    plt.tight_layout()
